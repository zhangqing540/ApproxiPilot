...
from main_operation.sobel import sobel
import queue
from basic_module.basic_module import dataset_in

if __name__ == '__main__':
    dataset_path="result/deepapprox/dataset_sobel_deepapprox_4w.txt"
    ...
    results = pool.starmap(sobel,input_data)
    pool.close()
    pool.join()
    while not output_q.empty():
        results.append(output_q.get())
    with open('result/deepapprox/dataset_sobel_deepapprox_4w_out.txt', 'w') as f:
        for r in results:
            f.write(str(r) + '\n')
    print("end_all")