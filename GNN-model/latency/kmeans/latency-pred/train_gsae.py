...
model.load_state_dict(torch.load("checkpoint/gsae_kmeans_latency_import.pth"))
...
for epoch in range(num_epochs):
    ...
    for data in train_loader:
        ...
        optimizer.zero_grad()
        output = model(data)
        new_data = data.y.reshape(batch_size, 1)/20
        loss = criterion(output,new_data)
        loss.backward()
        optimizer.step()
        epoch_loss += loss.item()
    ave_loss=epoch_loss/14000
    scheduler.step(ave_loss)
    print('Epoch: {:03d}, Loss: {:.4f}'.format(epoch, ave_loss))
    torch.save(model.state_dict(), "checkpoint/gsae_kmeans_latency_import.pth")
